...
-(id)init
{
    if(self = [super init])
    {
        ...
        self.praiseLabel.preferredMaxLayoutWidth = [UIScreen mainScreen].bounds.size.width - 80;;
...
{
    float height=  [CommentCell sf_heightForTableView:tableView config:^(UITableViewCell *sourceCell) {
...
{
    self.commentDataArray = [NSMutableArray arrayWithArray:model.commentsModelArr];
    self.likeArray = [NSMutableArray arrayWithArray:model.likeNameArr];
    self.indexPath = indexPath;
    [self setPraiseLabelTextWithLikeArray:model.likeNameArr];
    [self.tableView reloadData];
    float height = 0.0;
    for(SF_CommentsModel *model in self.commentDataArray)
    {
        float cellHeight = [CommentCell sf_heightForTableView:self.tableView config:^(UITableViewCell *sourceCell) {
            CommentCell *cell = (CommentCell *)sourceCell;
